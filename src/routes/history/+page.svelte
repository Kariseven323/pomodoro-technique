<script lang="ts">
  ...
  import { exportHistory, getFocusAnalysis, getHistory, getInterruptionStats, setHistoryRemark } from "$lib/api/tauri";
  import type {
    DateRange,
    ExportField,
    ExportFormat,
  ...
  /** 当前展示用的“合并日列表”（历史 + 中断）。 */
  const combinedDays = $derived(mergeDays(days, interruptionDaysInRange($appData?.interruptions ?? [], range)));
  ...
  <div class="mx-auto w-full max-w-5xl space-y-6">
    <header class="flex items-center justify-between gap-3">
      <div>
        ...
        <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">日/周/月 · 热力图 · 备注 · 导出 · 分析</p>
      </div>
      <button
        type="button"
        class="rounded-2xl bg-zinc-900 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-zinc-800 disabled:opacity-40 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-100"
        onclick={openExport}
    ...
    {#if exportError}
      <div class="rounded-2xl bg-red-500/10 p-3 text-sm text-red-600 dark:text-red-300">导出失败：{exportError}</div>
    {/if}
    {#if lastExportPath}
      <div class="rounded-2xl bg-emerald-500/10 p-3 text-sm text-emerald-700 dark:text-emerald-300">
        已导出：{lastExportPath}
      </div>
    {/if}
    ...
    <div class="rounded-2xl bg-white p-4 shadow-sm dark:bg-zinc-900">
      <div class="flex items-center justify-between gap-3">
        ...
        <div class="flex rounded-2xl bg-black/5 p-1 dark:bg-white/10">
          <button
            type="button"
      ...
      {#if viewMode === "day"}
        <label class="mt-4 block">
          <div class="mb-1 text-xs text-zinc-500 dark:text-zinc-400">选择日期</div>
          <input
            class="w-full rounded-2xl border border-black/10 bg-white px-3 py-2 text-sm text-zinc-900 outline-none dark:border-white/10 dark:bg-zinc-900 dark:text-zinc-50"
            type="date"
      ...
      {:else if viewMode === "month"}
        <label class="mt-4 block">
          <div class="mb-1 text-xs text-zinc-500 dark:text-zinc-400">选择月份</div>
          <input
            class="w-full rounded-2xl border border-black/10 bg-white px-3 py-2 text-sm text-zinc-900 outline-none dark:border-white/10 dark:bg-zinc-900 dark:text-zinc-50"
            type="month"
    ...
    <div class="rounded-2xl bg-white p-4 shadow-sm dark:bg-zinc-900">
      <div class="mb-3 text-sm font-medium text-zinc-900 dark:text-zinc-50">今日统计</div>
      <div class="grid grid-cols-3 gap-3">
        <div class="rounded-2xl bg-black/5 p-3 dark:bg-white/10">
          <div class="text-xs text-zinc-500 dark:text-zinc-400">完成</div>
          <div class="mt-1 text-xl font-semibold text-zinc-900 tabular-nums dark:text-zinc-50">
            {$timerSnapshot?.todayStats.total ?? 0}
          </div>
        </div>
        <div class="rounded-2xl bg-black/5 p-3 dark:bg-white/10">
          <div class="text-xs text-zinc-500 dark:text-zinc-400">中断</div>
          <div class="mt-1 text-xl font-semibold text-zinc-900 tabular-nums dark:text-zinc-50">
            {todayInterruptionCount()}
          </div>
        </div>
        <div class="rounded-2xl bg-black/5 p-3 dark:bg-white/10">
          <div class="text-xs text-zinc-500 dark:text-zinc-400">连击</div>
          <div class="mt-1 text-xl font-semibold text-zinc-900 tabular-nums dark:text-zinc-50">
            {$appData?.currentCombo ?? 0}
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="rounded-2xl bg-white p-4 shadow-sm dark:bg-zinc-900">
      ...
      <div class="max-h-[520px] space-y-3 overflow-auto pr-1" onscroll={onScroll}>
        ...
        {:else}
              ...
              {#if expandedDates.has(d.date)}
                <div class="mt-3 space-y-2">
                      ...
                      <div class="flex items-center justify-between gap-3">
                        <div class="min-w-0">
                          <div class="truncate font-medium text-zinc-900 dark:text-zinc-50">
                            {r.tag || "未分类"} · {phaseText(r.phase)}
                          </div>
                          <div class="mt-1 text-xs text-zinc-600 dark:text-zinc-300">
                            {r.startTime} - {derivedEndTime(r)} · {r.duration} 分钟
                          </div>
                        </div>
                        <div class="text-right text-xs text-zinc-600 dark:text-zinc-300">
                          {formatMmSs(BigInt(r.duration) * 60n)}
                        </div>
                      </div>

                      <div class="mt-2">
                        <div class="mb-1 text-xs text-zinc-500 dark:text-zinc-400">备注</div>
                        <div class="flex items-center gap-2">
                          <input
                            ...
                            oninput={(e) => onRemarkInput(d.date, i, (e.currentTarget as HTMLInputElement).value)}
                          />
                          <button
                            type="button"
                  ...
                  {#if d.interruptions.length > 0}
                    <div class="mt-3">
                      <div class="mb-2 text-xs font-medium text-zinc-500 dark:text-zinc-400">中断记录</div>
                      <div class="space-y-2">
                        {#each d.interruptions as it, ii (it.timestamp + ii)}
                          ...
                          >
                            <div class="min-w-0">
                              ...
                              <div class="mt-1 text-xs text-zinc-600 dark:text-zinc-300">
                                {hhmmFromIso(it.timestamp)} · 已专注 {formatMmSs(it.focusedSeconds)} · 剩余 {formatMmSs(
                                  it.remainingSeconds,
                                )}
                              </div>
          ...
          {/each}
          {#if loadingMore}
            <div class="text-sm text-zinc-500 dark:text-zinc-400">加载更多...</div>
          {/if}
          {#if viewMode === "week" && !hasMore && combinedDays.length > 0}
            <div class="text-xs text-zinc-500 dark:text-zinc-400">已加载全部历史</div>
          {/if}
        {/if}
      </div>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
      <div class="rounded-2xl bg-white p-4 shadow-sm dark:bg-zinc-900">
        <div class="mb-2 text-sm font-medium text-zinc-900 dark:text-zinc-50">专注分析</div>
        <FocusAnalysisView {analysis} loading={analysisLoading} error={analysisError} />
      </div>
      <div class="rounded-2xl bg-white p-4 shadow-sm dark:bg-zinc-900">
        <div class="mb-2 text-sm font-medium text-zinc-900 dark:text-zinc-50">中断分析</div>
