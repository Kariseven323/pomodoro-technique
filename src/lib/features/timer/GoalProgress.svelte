...
<div class="space-y-3">
  <div
    class="rounded-2xl border border-white/20 bg-white/70 p-4 shadow-sm backdrop-blur-xl dark:border-white/10 dark:bg-white/5"
  >
    <div class="flex items-center justify-between text-sm">
      <div class="font-medium text-zinc-900 dark:text-zinc-50">今日目标</div>
      <div class="text-zinc-700 tabular-nums dark:text-zinc-200">
    ...
    <div class="mt-2 h-2 overflow-hidden rounded-full bg-black/5 dark:bg-white/10">
      <div
        ...
        style={widthStyle(props.progress.dailyCompleted, props.progress.dailyGoal)}
      ></div>
    </div>
    {#if props.progress.dailyGoal > 0}
      <div class="mt-2 text-xs text-zinc-600 dark:text-zinc-300">
        {percent(props.progress.dailyCompleted, props.progress.dailyGoal)}%
      </div>
    {/if}
  </div>
  ...
  >
    ...
    <div class="mt-2 h-2 overflow-hidden rounded-full bg-black/5 dark:bg-white/10">
      <div
        ...
        style={widthStyle(props.progress.weeklyCompleted, props.progress.weeklyGoal)}
      ></div>
    </div>
    {#if props.progress.weeklyGoal > 0}
      <div class="mt-2 text-xs text-zinc-600 dark:text-zinc-300">
        {percent(props.progress.weeklyCompleted, props.progress.weeklyGoal)}%
      </div>
    {/if}
  </div>
</div>
